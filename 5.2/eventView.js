...
class EventsManager {
    ...
    eds_service_found(connection, name, name_owner) {
        ...
        if (this._calendar_server == null) {
            log("calendar@akenejie: Calendar events supported.")
    ...
    log_dbus_error(e) {
        global.logError(`calendar@akenejie: Could not check for calendar event support: ${e.toString()}`);
    }
...
class EventList {
    ...
    set_date_format_callback(callback, format_string) {
        this._date_format_callback = callback;
        this._date_format_string = format_string;
        if (this.selected_date) {
            this.set_date(this.selected_date);
        }
    }

    set_date(gdate) {
        if (typeof this._date_format_callback === 'function' && this._date_format_string) {
            this.selected_date_label.set_text(this._date_format_callback(gdate, this._date_format_string));
        } else {
