...
class CinnamonCalendarApplet extends Applet.TextApplet {
    ...
    _formatDateString(gDateTime, formatStr) {
        ...
        return formatStr.replace(/%([-]?)([a-zA-Zｼｲﾄ%])/g, (match, modifier, code) => {
            let nPad = modifier !== '-';
    ...
    _updateFormatString() {
        ...
        this.mainsettings_format = main_string;
    ...
    _updateClockAndDate() {
        ...
        if (!label_string) {
            label_string = this._formatDateString(localDateTime, "~CLOCK FORMAT ERROR~ %l:%M %p");
        }
