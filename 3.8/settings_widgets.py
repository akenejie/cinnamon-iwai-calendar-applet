...
def list_edit_factory(params):
    ...
    kwargs = { 'label': gettext.dgettext(UUID, params['title']) }
    if is_combo:
...
class ClocksList(JSONSettingsList):
    def __init__(self, info, key, settings):
        ...
        for tz in pytz.common_timezones:
            ...
            if region not in self.region_map:
                ...
                self.region_list.append((region, gettext.dgettext(UUID, region)))
            self.region_map[region].append((city, gettext.dgettext(UUID, city)))
    ...
    def open_add_edit_dialog(self, info=None):
        if info is None:
            ...
            title = gettext.dgettext(UUID, "Add new entry")
        else:
            r, c = self.split_tz(info[1])
            data = { "label": info[0], "region": r, "city": c }
            title = gettext.dgettext(UUID, "Edit entry")
        ...
        columns = [
            ...
            {"id": "region", "title": gettext.dgettext(UUID, "Region"), "options": self.region_list},
            {"id": "city", "title": gettext.dgettext(UUID, "City"), "options": self.region_map[data['region']]}
        ]
